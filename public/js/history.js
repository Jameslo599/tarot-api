...
document.querySelector("#delete-all").addEventListener("click", async () => {
  try {
    if (confirm("Delete all saved readings?")) {
      await fetch(`https://tarotapi.up.railway.app/${session}/delete-all`, {
        method: "DELETE",
      });
  ...
  try {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
      ...
      arr[i].removeState = async function () {
        //Delete reading
        await fetch(
          `https://tarotapi.up.railway.app/${session}/delete/${this.id}`,
          {
            method: "DELETE",
          }
        );
...
function makeSpan(query, date, fn) {
  ...
  link.href = "https://tarotapi.up.railway.app/card-reading";
  link.target = "_blank";
  ...
  link.addEventListener("click", async () => {
    try {
      ...
      await fetch(`https://tarotapi.up.railway.app/${user}/view/${fn.id}`);
    } catch (error) {
      console.log(error);
    }
  });
